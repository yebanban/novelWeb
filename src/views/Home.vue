<template>
  <div py-10 mx-10 md:m-x-60 :class="loading ? 'blur-sm' : ''">
    ...
    <div mt-10 bg="light-100/50" min-h="[78.5vh]" rounded-2xl font="sans" shadow="xl blue-100" w="full" box-border
      p="y-10 x-15" grid='~ sm:cols-1 md:cols-2 lg:cols-3 xl:cols-4 2xl:cols-5 gap-8'>
      <div v-for="book in books">
        <div border="1 gray-300" rounded-md p-2 flex justify="center" items-center cursor-pointer
    ...
    <input-dialog v-model="dialogVisible" @clickEnter="newBook" title="新建小说" placeholder="请输入新小说名" />
  </div>
  <div v-if="isShowMenu" p="y-1 " w-25 border="1 gray-400" shadow="sm gray-900/10" rounded-md absolute z-10 bg="white"
    :style="{ top: menuTop, left: menuLeft }">
    <div v-for="item in menus" font-mono text="sm center gray-900" p-y-1 p-x-3 hover-bg="blue-500/20"
      @click="item.clickMe" select-none cursor-pointer>
      {{ item.name }}
    </div>
  </div>
  <input-dialog v-model="updateDialogVisible" @clickEnter="updateBook" title="修改小说名" placeholder="请输入小说名" />
